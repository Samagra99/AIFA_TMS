                            ...
                            return (
                              <button key={ex.id} onClick={() => onSelectExercise?.(ex)}
                                ...
                                <div className="flex gap-1 shrink-0 px-2">
                                  <span className="text-[9px] font-bold uppercase text-slate-500 border border-slate-200 dark:border-slate-700 px-1 rounded bg-slate-100 dark:bg-slate-800">
                                    {ex.default_flight_type === 'solo' ? 'SOLO' : 'DUAL'}
                                  </span>
                                  {ex.default_is_cross_country && <span className="text-[9px] font-bold uppercase text-amber-600 border border-amber-200 px-1 rounded bg-amber-50 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-400">XC</span>}
                                  {ex.default_is_night && <span className="text-[9px] font-bold uppercase text-indigo-600 border border-indigo-200 px-1 rounded bg-indigo-50 dark:border-indigo-900 dark:bg-indigo-950 dark:text-indigo-400">NIGHT</span>}
                                  {ex.default_is_simulator && <span className="text-[9px] font-bold uppercase text-blue-600 border border-blue-200 px-1 rounded bg-blue-50 dark:border-blue-900 dark:bg-blue-950 dark:text-blue-400">FSTD</span>}
                                  {(ex.default_is_instrument_actual || ex.default_is_instrument_simulated) && <span className="text-[9px] font-bold uppercase text-slate-600 border border-slate-300 px-1 rounded bg-slate-100 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300">IFR</span>}
                                  {ex.is_ground_event && <span className="text-[9px] font-bold uppercase text-emerald-600 border border-emerald-200 px-1 rounded bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-400">GRND</span>}
                                </div>
                                {grade && (
